...
`incident_id` 只用于页面说明和返回 Overview 的链接，不发送到操作创建 API，不影响候选计划、幂等键或授权。用户若修改已预填的目标或端口，这些值只会作为一次普通操作请求重新接受合格 peer、最新诊断和计划一致性校验；系统不声称仍在处理原 incident。
...
- [URL 预填值被修改] → 只作为表单默认值；服务端继续校验 peer、最新诊断、计划和授权，且产品不把修改后的请求标记为原 incident 的权威处理结果。
