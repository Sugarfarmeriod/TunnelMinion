...
官方 WireGuard 配置只能指定 `ListenPort`，不能绑定单个本机 IP；因此用户已明确授权 B 的独立
测试接口在测试期间监听 `*:18889/udp`。该授权不包含其他端口、防火墙修改或端口转发；Murus
和现有网络边界继续决定实际可达性，删除测试接口后该监听必须消失。
...
- [WireGuard 监听端口作用于全部本机地址] → 签名配置与本机授权共同绑定唯一 UDP 端口；首轮
  仅允许 B 的 `18889`，不修改 Murus/端口转发，清理后验证监听消失。
