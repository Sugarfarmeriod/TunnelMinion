...
- [x] 1.3 以包体积、可访问性、维护成本和许可证验证轻量自有 tokens 与少量无样式可访问 primitives；浏览器门禁固定 Chromium + WebKit
...
- [x] 1.6 确认节点/服务首页首版是否加入搜索与排序；未获得新产品决定时固定采用小型家庭/实验室规模的分组列表
- [x] 1.7 运行 spike、依赖许可证/漏洞、生成物秘密、OpenSpec strict 和 Python 全量门禁，提交并推送工具链阶段
...
- [x] 3.3 由同一应用装配 owner 抽取 Windows/macOS 共用 Coordinator/path helper，连接真实 status/cache/path/evidence/authorization，避免已配置状态误报 `unconfigured`
  - 证据（2026-08-24）：Windows/macOS 应用装配已把同一个 managed-path status provider 同时接入资源路由与 Overview，并在 provider 返回 `None` 时回退旧 Coordinator/path binding；双平台工厂测试分别覆盖 managed 状态优先与旧 binding 回退。后端全量 `1332 passed, 6 skipped` 且 statement/branch 100%，前端 89 项单元测试、5 项供应链测试及 Chromium/WebKit 36 项浏览器测试通过。
...
- [x] 3.7 在 Windows/macOS 开发运行中验收总览解释、加载/刷新延迟和初始 JS+CSS gzip 不超过 300 KiB，提交并推送 Overview 阶段
  - 证据：PR #40 的持续集成 run `31490855600` 在 `9a90917` 上由 Windows Chromium 与 macOS WebKit 通过真实 FastAPI 开发运行门禁；`evaluations/baselines/local-product-interface-v1.json` 固定双平台总览首次加载/刷新延迟及初始 JS+CSS `145073` bytes（`141.67 KiB`），低于 `300 KiB`，对应可执行复核为 `tests/evaluation/test_local_product_interface_baseline.py`。
...
- [x] 4.5 运行现有 AI 评估与前后端门禁，提交并推送 Chat 阶段
