...
- [ ] 4.1 实现逐节点能力完整快照、工具版本/平台/风险/可用性/schema 哈希和原子替换
- [ ] 4.2 实现稳定 service ID、逐节点服务完整快照、来源/置信度/时间和 stopped 收敛
- [ ] 4.3 实现 snapshot ID、本地单调序号、幂等键、乱序拒绝和单调 server revision
- [ ] 4.4 实现基于节点状态与服务器时间的能力/服务 fresh、stale、offline、revoked 传播
- [ ] 4.5 实现按节点、状态、平台、工具版本、协议、端口、可访问性和新鲜度过滤的有界稳定分页查询
- [ ] 4.6 覆盖重复/乱序/并发/超限快照、服务消失、节点离线、跨 network 游标和一致读取测试
- [ ] 4.7 运行目录收敛、隔离、性能、事务和数据最小化门禁，提交并推送目录阶段
