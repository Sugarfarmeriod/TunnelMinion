...
防火墙日志采用“可选增强、不可依赖”的诊断策略：当部署者已经提供安全的只读日志接口和授权时，
验收或故障诊断可以保存时间、动作、进程/端口和规则标识的脱敏摘要，用来辅助解释
`peer_unreachable`；日志不可用时只报告稳定的 peer 结果与“可能涉及防火墙、路由、监听或网络”的
通用提示，不把原因武断归咎于防火墙。跨厂商日志采集适配器不属于本 change。
