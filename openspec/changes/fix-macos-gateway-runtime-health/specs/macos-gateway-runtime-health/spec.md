...
#### Scenario: 防火墙日志可用时提供可选诊断

- **WHEN** 部署者已提供安全的只读防火墙日志接口和读取授权，且 peer 请求失败或超时
- **THEN** 验收或诊断工具 SHOULD 只保存时间、动作、进程/端口、规则标识和大小的脱敏摘要，不保存
  完整规则、数据包正文、token、私钥或其他秘密

#### Scenario: 没有 Murus 或无法读取防火墙日志

- **WHEN** 机器没有安装 Murus、使用其他防火墙、日志接口不存在或当前账户没有读取权限
- **THEN** 本地 lifecycle 与 peer 验收 MUST 继续运行，以真实跨机器请求报告 `peer_reachable` 或
  `peer_unreachable`；系统只给出防火墙、路由、监听或网络等通用可能原因，不得要求安装特定产品，
  也不得把失败武断归因于防火墙
