...
- [x] 5.2 在常规本地应用 lifespan 中托管唯一 lifecycle，未配置/enrollment-required/awaiting-authorization 时保持零写入且不改变环回绑定
  - 当前证据：`managed_application_lifespan` 对 start 失败、stop 失败均保证 close，`ManagedPathApplication.close()` 幂等；`test_lifespan_closes_once_when_start_or_stop_fails` 证明 start/stop 异常各恰好 close 一次。`test_common_factory_keeps_awaiting_authorization_before_provider_apply`、双平台常规入口与完整安全矩阵继续证明 awaiting-authorization/能力降级不 observe/apply，未触碰真实网络或环回绑定。
