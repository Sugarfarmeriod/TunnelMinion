...
归档 change `2026-07-31-integrate-managed-node-runtime` 已将 managed config 治理、Provider 执行、路径状态和常规入口标为完成，但当前生产运行时只拉取、验签并保存 pending：它不读取既有 L3 授权、不调用 Provider，且仓库没有生产 `PathProbe` 或 Windows/macOS 常规入口的真实 path lifecycle。因而 `improve-local-product-experience` 只能接入 Coordinator/cache 和陈旧占位 evidence，不能真实完成任务 3.3。

## What Changes

- 为 Windows 与 macOS 提供确定性、只读的平台 `PathProbe`，从实时系统事实形成有时间边界的 endpoint、handshake、host route 和 target probe 证据。
- 建立一条共享的 managed path lifecycle，把既有 signed config 同步、持久化 L3 授权、Provider governance、Provider、`DirectPathVerifier` 和 `DirectPathController` 串成单并发生命周期，并持久化可恢复的脱敏 selection/evidence 状态。
- 未找到与 network/node/revision/计划摘要精确匹配的既有本机 L3 授权时，只保存 pending 并显示 `awaiting-authorization`；普通启动、模型、对话、记忆和服务观察均不得创建授权或触发网络写入。
...
- 复用现有 `managed-network-provider`、`operation-policy`、Provider plan/apply/verify/rollback/recover、所有权账本和本机持久化授权；不改变其写入权限与资源边界。
