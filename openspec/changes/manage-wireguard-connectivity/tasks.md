...
- [ ] 10.2 实现签名配置与本机 L3 授权共同绑定的精确 `/32` 宽路由例外和 UDP listen port 门禁；在隔离数据目录生成本机私钥和所有权账本，保留原 Mihomo 路由，先创建临时监听 `*:18889/udp` 的 B，再创建 A 的独立 managed 测试接口，不切换生产 endpoint；清理后监听必须消失
