...
- [ ] 9.1 建立地址冲突、签名篡改、跨节点重放、策略扩权、部分成功、回滚失败、所有权冲突和控制面离线数据集
- [ ] 9.2 评估配置收敛正确率、direct/relay 选择正确率、错误参数率、安全拦截率、回滚成功率、切换时间、延迟和资源成本
- [ ] 9.3 证明模型启用/禁用不改变 Provider 计划、授权、执行、验证和回滚结果；记录模型解释的 token/延迟/成本但不把它作为网络正确性
- [ ] 9.4 扩展威胁模型、数据分类、ADR、架构、运维、恢复、卸载和人工干预手册
- [ ] 9.5 扩展安全扫描与导出/卸载测试，禁止私钥、完整配置、认证头、用户 route 和未过滤 endpoint 泄漏
- [ ] 9.6 建立 OpenSpec 场景、自动测试、隔离环境证据、指标和任务对照
- [ ] 9.7 运行格式、严格类型、100% 源/分支覆盖、全量测试、安全扫描、OpenSpec、性能和协议兼容门禁
- [ ] 9.8 提交并推送评估与文档阶段
