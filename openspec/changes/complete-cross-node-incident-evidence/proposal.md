...
- 模型设置仍只激活一个 Provider，但保留已验证的非秘密配置供切回，并按规范化 endpoint 隔离本机 API key，避免 Qwen 与 DeepSeek 相互覆盖或跨 endpoint 复用凭据。
- 增加固定、无秘密的隔离跨节点验收，覆盖 Windows 请求 macOS、目标身份/能力冲突、远端失败和零写操作；在当前提交与固定数据集上产出 Windows/macOS 平台凭据。
- 生成版本化的最终指标冻结清单，绑定提交、数据集、Prompt、模型、工具协议、平台凭据和安全/质量门槛；只在确定性门禁通过后运行一次用户选定的 DeepSeek 最终评测，并保留先前 Qwen 失败或中断尝试。
...
- `model-provider-configuration`: 保留可切回的非秘密模型配置，并把 API key 绑定到对应 endpoint，而不是所有模型共用一个密钥槽。

## Impact

- 影响 Incident 调查编排、static Gateway peer 解析、模型设置、Windows/macOS 应用组装、固定评测与平台验收脚本。
