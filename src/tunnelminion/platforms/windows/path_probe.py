"""Windows 官方只读事实上的 managed PathProbe。"""
...
class WindowsPathProbe(PlatformPathProbe):
    """读取官方 `wg show`、固定 host route 和私有目标连接状态。"""
    ...
    def _preflight_error_code(
        preflight: WindowsProviderPreflight | None,
    ...
    def _snapshot_error(error_code: str | None) -> DirectPathErrorCode | None:
        if error_code is None:
            return None
        if "permission" in error_code or error_code in {"service_query_failed"}:
            return DirectPathErrorCode.PERMISSION_DENIED
        return DirectPathErrorCode.UNSUPPORTED
