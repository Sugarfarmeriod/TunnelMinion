...
本台账只定义素材职责和采集门禁，不代表素材已经生成。当前结构复核基线为 `origin/main@b21b00e68e2298bb5db6a5c75d3e8629a33e4d05`；“素材来源”必须填写最终采集时对应的稳定 `main` SHA，不能用工作包提交或 fixture 代替。
...
| `screenshot-overview` | 设备、服务和访问地址入口 | `planned` | 未采集：完整访问地址仍缺失 | 平台、窗口尺寸与数据集待记录 | 来源页面与截图一一对应 | 地址和节点名称使用获准的脱敏值 |
| `screenshot-operation` | 候选、目标节点批准、执行与恢复状态 | `planned` | 未采集：等待同一稳定数据的最终素材阶段 | 平台与 operation ID 待记录 | 与公开事件和 Evidence 引用对齐 | 不显示未授权操作已生效 |
...
| `product-surface-review` | Overview、Chat、Operations、Memories、Settings 对主演示与深挖的承载复核 | `main-verified`，3.2 已完成 | `b21b00e68e2298bb5db6a5c75d3e8629a33e4d05` | repository / `verify_product_surface_review.py` | `product-surface-review.md` → 最小补强计划 | 不把端口冒充完整访问地址；不新增展示专用后端 |
