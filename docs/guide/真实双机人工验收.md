...
Murus 规则正文没有稳定的仓库内只读接口，因此自动报告只给出 `automated_passed`。操作者需要在
执行前后从同一 Murus 视图或导出物生成 SHA-256；自动门禁通过后再完成报告：

```powershell
uv run python scripts/run_managed_node_runtime_ab_acceptance.py `
  --ssh-target 10.77.0.1 `
  --finalize-report evaluations/platform/managed-node-runtime-real-ab-2026-07-31.json `
  --murus-before-sha256 <BEFORE_SHA256> `
  --murus-after-sha256 <AFTER_SHA256>
```

只有自动门禁通过且 Murus 前后哈希完全一致时，最终 `passed` 才为 `true`。历史真机授权与历史
哈希不能代替本轮确认。
